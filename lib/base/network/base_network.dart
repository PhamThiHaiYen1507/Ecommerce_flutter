...
import 'package:nws_yenpth1_ecommerce_flutter/base/network/app_connection.dart';
...
class BaseNetworkConfig {
  ...
  late final FutureOr<Map<String, dynamic>?> Function()? _defaultHeaders;
  late final FutureOr<String?> Function() _getAccessToken;
  late final FutureOr<String?> Function(Dio dio)? _getRefreshToken;

  static BaseNetworkConfig? _singleton;

  BaseNetworkConfig._internal(
    this._baseUrl,
    this._secure,
  ...
  static Future<void> initial({
    ...
    final FutureOr<Map<String, dynamic>?> Function()? defaultHeaders,
    FutureOr<String?> Function(Dio dio)? getRefreshToken,
  }) async {
    await AppConnection.init();
    _singleton ??= BaseNetworkConfig._internal(
      baseUrl,
      secure,
      defaultHeaders,
      getAccessToken,
...
abstract class BaseNetWork {
    ...
    return QueuedInterceptorsWrapper(
      onRequest: (options, handler) async {
        if (options.extra['auth'] == true) {
          options.headers['Authorization'] = await _accessToken;
        }
  ...
  FutureOr<String?> get _accessToken {
    return BaseNetworkConfig._singleton?._getAccessToken();
  }
